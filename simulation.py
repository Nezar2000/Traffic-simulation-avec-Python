...
class Road:
    """Classe represente une route, qui est un segment entre deux intersections"""

    def __init__(self, cross1, cross2, speed_limit, id=None):
        """Initialiser un objet Road connecte a deux Intersections, avec un speed_limit"""
    ...
    def incoming_veh(self, veh, origin_cross, x = 0):
        """Vehicule entrant sur la route en provenance de origin_cross
         x est la position du vehicule sur la route a son arrivee
         (utile lorsque les vehicules viennent d'une autre route avec une certaine vitesse)"""
...
class Cross:
    """Classe modelisant une cross aux coordonnees (x,y), avec ou sans feux de circulation"""

    def __init__(self, coords, id=None, traffic_lights=True):
        """Generer une cross Cross"""
    ...
    def updateTrafficLights(self, t):
        """Verifiez l'heure et mettez a jour les feux tricolores en fonction de l'avancement du cycle"""
...
class Vehicle:
    ...
    def __init__(self, road, origin_cross, T = 1, s0 = 2, a = 1.5, vehicle_type = "car", b = 1.5):
        """road : Route sur laquelle la voiture est convoquee
         origin_cross : Traverser par où la voiture entre sur la route
         T : Avancement souhaite [s]
         leader: vehicule devant
         s0 = distance minimale (bumper-to-bumper) avec le leader [m]
         vehicule_type = voiture, camion
         b = deceleration confortable du conducteur, b > 0 [m/s²]
        """
        # Nous verifions que les parametres d'entree ont les types attendus
        if type(road) is not Road:
            raise NotRoadError
